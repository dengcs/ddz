	...
	public class PulleyJoint extends JointBase {
		...
		override protected function _createJoint():void {
			if (!_joint) {
				...
				var posA:Point = (otherBody.owner as Sprite).localToGlobal(Point.TEMP.setTo(otherAnchor[0], otherAnchor[1]), false, Physics.I.worldRoot);
				var anchorVecA:* = new box2d.b2Vec2(posA.x / Physics.PIXEL_RATIO, posA.y / Physics.PIXEL_RATIO);
				var posB:Point = (selfBody.owner as Sprite).localToGlobal(Point.TEMP.setTo(selfAnchor[0], selfAnchor[1]), false, Physics.I.worldRoot);
				var anchorVecB:* = new box2d.b2Vec2(posB.x / Physics.PIXEL_RATIO, posB.y / Physics.PIXEL_RATIO);
				var groundA:Point = (otherBody.owner as Sprite).localToGlobal(Point.TEMP.setTo(otherGroundPoint[0], otherGroundPoint[1]), false, Physics.I.worldRoot);
				var groundVecA:* = new box2d.b2Vec2(groundA.x / Physics.PIXEL_RATIO, groundA.y / Physics.PIXEL_RATIO);
				var groundB:Point = (selfBody.owner as Sprite).localToGlobal(Point.TEMP.setTo(selfGroundPoint[0], selfGroundPoint[1]), false, Physics.I.worldRoot);
