package laya.d3.graphics {
	...
	public class StaticBatchManager {
		 ...
		 * @private
		 */
		private static function _addToStaticBatchQueue(sprite3D:Sprite3D, renderableSprite3D:Vector.<RenderableSprite3D>):void {
			...
			for (var i:int = 0, n:int = sprite3D.numChildren; i < n; i++)
				_addToStaticBatchQueue(sprite3D._children[i] as Sprite3D, renderableSprite3D);
		}
		...
		public static function combine(staticBatchRoot:Sprite3D, renderableSprite3Ds:Vector.<RenderableSprite3D> = null):void {
			...
			if (!renderableSprite3Ds) {
				renderableSprite3Ds = new Vector.<RenderableSprite3D>();
				if (staticBatchRoot)
					_addToStaticBatchQueue(staticBatchRoot, renderableSprite3Ds);
			}
			
			var batchSpritesCount:int = renderableSprite3Ds.length;
			if (batchSpritesCount > 0) {
				for (var i:int = 0; i < batchSpritesCount; i++) {
				...
				for (var k:int = 0, m:int = _managers.length; k < m; k++) {
