package laya.d3.utils {
	...
	public class Utils3D {
		 ...
		 * @private
		 */
		private static function _createSprite3DInstance(nodeData:Object, spriteMap:Object, outBatchSprites:Vector.<RenderableSprite3D>):Node {
			var node:Node;
			switch (nodeData.type) {
			case "Scene3D": 
				node = new Scene3D();
			...
			if (childData) {
				for (var i:int = 0, n:int = childData.length; i < n; i++) {
					var child:* = _createSprite3DInstance(childData[i], spriteMap, outBatchSprites)
					node.addChild(child);
				}
			}
		 ...
		 * @private
		 */
		private static function _createComponentInstance(nodeData:Object, spriteMap:Object):void {
			...
			if (childData) {
				for (var i:int = 0, n:int = childData.length; i < n; i++)
					_createComponentInstance(childData[i], spriteMap)
			}
			...
			if (componentsData) {
				for (var j:int = 0, m:int = componentsData.length; j < m; j++) {
					var data:Object = componentsData[j];
					var clas:* = Browser.window.Laya[data.type];//兼容
					if (!clas) {//兼容
						var clasPaths:Array = data.type.split('.');
						clas = Browser.window;
						clasPaths.forEach(function(cls:*):void {
		 ...
		 * @private
		 */
		public static function _createNodeByJson02(nodeData:Object, outBatchSprites:Vector.<RenderableSprite3D>):Node {
			var spriteMap:Object = {};
			var node:Node = _createSprite3DInstance(nodeData, spriteMap, outBatchSprites);
			_createComponentInstance(nodeData, spriteMap);
			return node;
		}
		 ...
		 * @private
		 */
		public static function _getHierarchyPath(rootSprite:Node, checkSprite:Node, path:Array):Array {
			path.length = 0;
		 ...
		 * @private
		 */
		public static function _getNodeByHierarchyPath(rootSprite:Node, invPath:Array):Node {
			var sprite:Node = rootSprite;
			for (var i:int = invPath.length - 1; i >= 0; i--) {
		 ...
		 * @private
		 */
		public static function _createNodeByJson(nodeData:Object, outBatchSprites:Vector.<RenderableSprite3D>):Node {//兼容代码
			var node:Node;
			switch (nodeData.type) {
			case "Scene3D": 
				node = new Scene3D();
