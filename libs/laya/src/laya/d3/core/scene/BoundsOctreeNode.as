package laya.d3.core.scene {
	...
	public class BoundsOctreeNode {
		...
		private function _checkAddNode(object:IOctreeObject):BoundsOctreeNode {
			...
			if (_children == null) {
				...
				for (var i:int = _objects.length - 1; i >= 0; i--) {//已有新子节点,检查已经存在的物体是否更适合子节点
					...
					var fitChildIndex:int = _bestFitChild(existObject.bounds.getCenter());
					if (_encapsulates(_getChildBound(fitChildIndex), existObject.bounds._getBoundBox())) {
			...
			var newFitChildIndex:int = _bestFitChild(object.bounds.getCenter());
			if (_encapsulates(_getChildBound(newFitChildIndex), object.bounds._getBoundBox()))
		...
		private function _addUp(object:IOctreeObject):Boolean {
			if ((CollisionUtils.boxContainsBox(_bounds, object.bounds._getBoundBox()) === ContainmentType.Contains)) {
				_add(object);
				return true;
			} else {
				if (_parent)
		...
		public function _update(object:IOctreeObject):Boolean {
			if (CollisionUtils.boxContainsBox(_bounds, object.bounds._getBoundBox()) === ContainmentType.Contains) {//addDown
				var addNode:BoundsOctreeNode = _checkAddNode(object);
		...
		public function add(object:IOctreeObject):Boolean {
			if (!_encapsulates(_bounds, object.bounds._getBoundBox())) //如果不包含,直接return false
				return false;
			_add(object);
			return true;
		}
		...
		public function shrinkIfPossible(minLength:int):BoundsOctreeNode {
			...
			for (var i:int = 0, n:int = _objects.length; i < n; i++) {
				var object:IOctreeObject = _objects[i];
				var newBestFit:int = _bestFitChild(object.bounds.getCenter());
				if (i == 0 || newBestFit == bestFit) {//判断所有的物理是否在同一个子节点中
					var childBounds:BoundBox = _getChildBound(newBestFit);
					if (_encapsulates(childBounds, object.bounds._getBoundBox()))
		...
		public function drawAllObjects(debugLine:PixelLineSprite3D, currentDepth:int, maxDepth:int):void {
			...
			for (var i:int = 0, n:int = _objects.length; i < n; i++)
				Utils3D._drawBound(debugLine, _objects[i].bounds._getBoundBox(), color);
