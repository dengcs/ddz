	...
	public class Scene3D extends Sprite implements ISubmit, ICreateResource {
		...
		public static function _parse(data:*, propertyParams:Object = null, constructParams:Array = null):Scene3D {
			...
			case "LAYAHIERARCHY:02": 
				scene = Utils3D._createNodeByJson02(json, outBatchSprits) as Scene3D;
				break;
			default: 
				scene = Utils3D._createNodeByJson(json, outBatchSprits) as Scene3D;
			}
		...
		/** @private */
		private var _lightmaps:Vector.<Texture2D> = new Vector.<Texture2D>();
		/** @private */
		...
		public function _clear(gl:WebGLContext, state:RenderContext3D):void {
			...
			var renderTarget:RenderTexture = camera.getRenderTexture();
		...
		public function _renderScene(gl:WebGLContext, state:RenderContext3D, customShader:Shader3D = null, replacementTag:String = null):void {
			...
			camera.getRenderTexture() ? _opaqueQueue._render(state, true, customShader, replacementTag) : _opaqueQueue._render(state, false, customShader, replacementTag);//非透明队列
			...
			camera.getRenderTexture() ? _transparentQueue._render(state, true, customShader, replacementTag) : _transparentQueue._render(state, false, customShader, replacementTag);//透明队列
		...
		override public function _parse(data:Object, spriteMap:Object):void {
			...
			if (lightMapsData) {
				...
				var lightmaps:Vector.<Texture2D> = new Vector.<Texture2D>(lightMapCount);
		...
		public function setlightmaps(value:Vector.<Texture2D>):void {
			var maps:Vector.<Texture2D> = _lightmaps;
			for (var i:int = 0, n:int = maps.length; i < n; i++)
		...
		public function getlightmaps():Vector.<Texture2D> {
			return _lightmaps.slice();//slice()防止修改数组内容
		}
		...
		override public function render(ctx:Context, x:Number, y:Number):void {
			//TODO:外层应该设计为接口调用
			ctx._curSubmit = Submit.RENDERBASE;//打断2D合并的renderKey
