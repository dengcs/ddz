	...
	public class Sprite3D extends Node implements ICreateResource {
		...
		public static function _parse(data:*, propertyParams:Object = null, constructParams:Array = null):Sprite3D {
			...
			case "LAYAHIERARCHY:02": 
				sprite = Utils3D._createNodeByJson02(json, outBatchSprits) as Sprite3D;
				break;
			default: 
				sprite = Utils3D._createNodeByJson(json, outBatchSprits) as Sprite3D;
			}
		...
		public static function instantiate(original:Sprite3D, parent:Node = null, worldPositionStays:Boolean = true, position:Vector3 = null, rotation:Quaternion = null):Sprite3D {
			var destSprite3D:Sprite3D = original.clone() as Sprite3D;
		 ...
		 */
		public override function _cloneTo(destObject:*, srcRoot:Node, dstRoot:Node):void {
			if (destroyed)
		 ...
		 * @private
		 */
		private static function _createSprite3DInstance(scrSprite:Sprite3D):Node {
			var node:Node = __JS__("new scrSprite.constructor()");
			var children:Array = scrSprite._children;
			for (var i:int = 0, n:int = children.length; i < n; i++) {
				var child:* = _createSprite3DInstance(children[i])
				node.addChild(child);
			}
			return node;
		}
		 ...
		 * @private
		 */
		private static function _parseSprite3DInstance(srcRoot:Node, dstRoot:Node, scrSprite:Node, dstSprite:Node):void {
			//scrSprite._cloneTo(dstSprite,srcRoot,dstRoot);//TODO:因为根据名字找Owner,子节点名字还未赋值有BUG
			var srcChildren:Array = scrSprite._children;
			var dstChildren:Array = dstSprite._children;
			for (var i:int = 0, n:int = srcChildren.length; i < n; i++)
				_parseSprite3DInstance(srcRoot, dstRoot, srcChildren[i], dstChildren[i])
		...
		public function clone():Node {
			var dstSprite3D:Node = _createSprite3DInstance(this);
			_parseSprite3DInstance(this, dstSprite3D, this, dstSprite3D);
			return dstSprite3D;
		}
