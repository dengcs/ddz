package laya.d3.component {
	...
	public class Animator extends Component {
		...
		/**@private */
		private var _avatar:Avatar;//[兼容性API]
		
		/**
		 * 获取avatar。
		 * @return avator。
		 */
		public function get avatar():Avatar {//[兼容性API]
			return _avatar;
		}
		 ...
		 * @param value avatar。
		 */
		public function set avatar(value:Avatar):void {//[兼容性API]
			if (_avatar !== value) {
				_avatar = value;
				if (value) {
		 ...
		 * @private
		 */
		private function _getAvatarOwnersAndInitDatasAsync():void {//[兼容性API]
			for (var i:int = 0, n:int = _controllerLayers.length; i < n; i++) {
				var clipStateInfos:Vector.<AnimatorState> = _controllerLayers[i]._states;
				for (var j:int = 0, m:int = clipStateInfos.length; j < m; j++)
		 ...
		 * @private
		 */
		private function _isLinkSpriteToAnimationNode(sprite:Sprite3D, nodeName:String, isLink:Boolean):void {//[兼容性API]
			if (_avatar) {
		 ...
		 * @private
		 */
		private function _isLinkSpriteToAnimationNodeData(sprite:Sprite3D, nodeName:String, isLink:Boolean):void {//[兼容性API]
		 ...
		 * @return 是否关联成功。
		 */
		public function linkSprite3DToAvatarNode(nodeName:String, sprite3D:Sprite3D):Boolean {//[兼容性API]
		 ...
		 * @return 是否解除关联成功。
		 */
		public function unLinkSprite3DToAvatarNode(sprite3D:Sprite3D):Boolean {//[兼容性API]
