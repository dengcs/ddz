package laya.resource {
	...
	import laya.webgl.canvas.Path;
	import laya.webgl.canvas.WebGLCacheAsNormalCanvas;
	...
	public class Context {
		...
		public static var MAXCLIPRECT:Rectangle = null;
		...
		public static function __init__():void {
			MAXCLIPRECT=  new Rectangle(0,0, _MAXSIZE, _MAXSIZE);
		...
		public var _globalClipMatrix:Matrix = new Matrix(Context._MAXSIZE, 0, 0, Context._MAXSIZE, 0, 0);	//用矩阵描述的clip信息。最终的点投影到这个矩阵上，在0~1之间就可见。
		...
		private var defTexture:Texture=null;	//给fillrect用
		...
		public var isMain:Boolean = false;				// 是否是主context
		 ...
		 */
		private function _releaseMem(keepRT:Boolean=false):void{			
			if (!_submits)
				return;
		 ...
		 * @param	keepRT  是否保留rendertarget
		 */
		public function destroy(keepRT:Boolean=false):void{
			--_contextcount;
			sprite = null;
			_releaseMem(keepRT);
		...
		public function clear():void
		{
			...
			{//第一次
				...
				_mesh = MeshQuadTexture.getAMesh(isMain);
				meshlist.push(_mesh);
				_pathMesh = MeshVG.getAMesh(isMain);
				meshlist.push(_pathMesh);
				_triangleMesh = MeshTexture.getAMesh(isMain);
		 ...
		 * @param	textAlign
		 */
		public function fillBorderText(txt:*, x:Number, y:Number, fontStr:String, fillColor:String, borderColor:String, lineWidth:int, textAlign:String):void {
			//webgl绘制不了，需要解决
		...
		private function _fillRect(x:Number, y:Number, width:Number, height:Number, rgba:uint):void {
			...
			if ( _mesh.vertNum + 4 > _MAXVERTNUM) {
				_mesh = MeshQuadTexture.getAMesh(isMain);//创建新的mesh  TODO 如果_mesh不是常见格式，这里就不能这么做了。以后把_mesh单独表示成常用模式 
				meshlist.push(_mesh);
				sameKey = false;
			}
		...
		*/
		...
		public function _inner_drawTexture(tex:Texture, imgid:int, x:Number, y:Number, width:Number, height:Number, m:Matrix, uv:Array, alpha:Number, lastRender:Boolean):Boolean {
			...
			if (mesh.vertNum + 4 > _MAXVERTNUM) {
				mesh = _mesh = MeshQuadTexture.getAMesh(isMain);//创建新的mesh  TODO 如果_mesh不是常见格式，这里就不能这么做了。以后把_mesh单独表示成常用模式 
				meshlist.push(mesh);
		...
		*/
		 ...
		 * @param	alpha
		 */
		public function drawTextureWithTransform(tex:Texture, x:Number, y:Number, width:Number, height:Number, transform:Matrix, tx:Number, ty:Number, alpha:Number, blendMode:String, colorfilter:ColorFilter = null):void {
		 ...
		 * @param	target
		 */
		private function _flushToTarget(context:Context, target:RenderTexture2D):void {
		...
		public function drawTriangles(tex:Texture, x:Number, y:Number, vertices:Float32Array, uvs:Float32Array, indices:Uint16Array, matrix:Matrix, alpha:Number, color:ColorFilter, blendMode:String):void {
			...
			if (triMesh.vertNum + vertices.length / 2 > _MAXVERTNUM) {
				triMesh = _triangleMesh = MeshTexture.getAMesh(isMain);//创建新的mesh  TODO 如果_mesh不是常见格式，这里就不能这么做了。以后把_mesh单独表示成常用模式 
		...
		public function clipRect(x:Number, y:Number, width:Number, height:Number):void {
			...
			if (_clipRect.width >= Context._MAXSIZE) {
		...
		public function flush():int {
			...
			_mesh = MeshQuadTexture.getAMesh(isMain);	//TODO 不要这样。
			_pathMesh = MeshVG.getAMesh(isMain);
			_triangleMesh = MeshTexture.getAMesh(isMain);
