package laya.webgl.text {
	...
	import laya.webgl.text.CharRender_Canvas;
	import laya.webgl.text.CharRender_Native;
	...
	public class TextRender {
		...
		public function filltext(ctx:Context, data:String, x:Number, y:Number, fontStr:String, color:String, strokeColor:String, lineWidth:int, textAlign:String, underLine:int = 0):void {
			if (data.length <= 0)
				return;
			//以后保存到wordtext中
		...
		public function fillWords(ctx:Context, data:Vector.<HTMLChar>, x:Number, y:Number, fontStr:String, color:String, strokeColor:String, lineWidth:int):void {
			if (!data) return;
		...
		public function _fast_filltext(ctx:Context, data:WordText, htmlchars:Vector.<HTMLChar>, x:Number, y:Number, font:FontInfo, color:String, strokeColor:String, lineWidth:int, textAlign:int, underLine:int = 0):void {
				...
				if ( splitTex ) {
					...
					while(true){
						...
						if (ri.isSpace) {	// 空格什么都不做
							...
							if (!add) {
								var o1:Object = { texgen:(ri.tex as TextTexture).genID, tex:ri.tex, words:[] };	// 根据genid来减少是否释放的判断量
					...
					// 整句渲染，则只有一个贴图
					if (Render.isConchApp){
						sameTexData[0] = {texgen:(ri.tex as TextTexture).genID, tex:ri.tex, words:[{ ri: ri, x: 0, y: 0, w: ri.bmpWidth / fontScaleX, h: ri.bmpHeight / fontScaleY }]};
					}else{
						sameTexData[0] = {texgen:(ri.tex as TextTexture).genID, tex:ri.tex, words:[{ ri: ri, x: 1/fontScaleX, y: 0/fontScaleY, w: (ri.bmpWidth-2) / fontScaleX, h: (ri.bmpHeight-1) / fontScaleY }]}; // 为了避免边缘像素采样错误，内缩一个像素
					}
				}
		 ...
		 */
		protected function _drawResortedWords(ctx:Context, startx:int, starty:int, samePagesData:Array ):void {
			var isLastRender:Boolean = ctx._charSubmitCache && ctx._charSubmitCache._enbale;
			var mat:Matrix = ctx._curMat;
			var slen:int = samePagesData.length;
			for ( var id:int = 0; id < slen; id++ ) {
				var dt:* = samePagesData[id];
				if (!dt) continue;
				var pri:Array = dt.words;
				var pisz:int = pri.length;  		if (pisz <= 0) continue;
				var tex:TextTexture = (samePagesData[id].tex as TextTexture);
				for (var j:int = 0; j < pisz; j++) {
					...
					if (Render.isConchApp) {
						ctx._drawTextureM(tex.texture as Texture, startx +riSaved.x -ri.orix , starty + riSaved.y -ri.oriy, riSaved.w, riSaved.h, null, 1.0, ri.uv);
					}else
						ctx._inner_drawTexture(tex.texture as Texture, (tex.texture as Texture).bitmap.id,
		...
		public function hasFreedText(txts:Array):Boolean {
			var sz:int = txts.length;
			for (var i:int = 0; i < sz; i++) {
				var pri:* = txts[i];
				if (!pri) continue;
				var tex:TextTexture = (pri.tex as TextTexture);
		...
		/////// native ///////
		public function filltext_native(ctx:Context, data:String, htmlchars:Vector.<HTMLChar>, x:Number, y:Number, fontStr:String, color:String, strokeColor:String, lineWidth:int, textAlign:String, underLine:int = 0):void {
